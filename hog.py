...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        returned_score = digit_fn(opponent_score  % 10)(player_score)
        opponent_score = opponent_score // 10
        while opponent_score > 0:
            function = digit_fn(opponent_score % 10)
            opponent_score = opponent_score // 10
            returned_score = function(returned_score)
    return returned_score % 30
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:  
        if who == 0:
            ...
            score0 += take_turn(player_rolls, score0, score1, dice, goal)
            score0 += hog_pile(score0, score1)
        if who == 1:
            player_rolls = strategy1(score1, score0)
            score1 += take_turn(player_rolls, score1, score0, dice, goal)
            score1 += hog_pile(score1, score0)
        who = next_player(who)
        turn, message = say(score0, score1, turn)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 == score1:
        current_leader = None
    if score0 == score1 or last_leader == current_leader:
        message = None
        return current_leader, message 
    new_message = f"Player {current_leader} takes the lead by {abs(score0 - score1)}"
    return current_leader, new_message
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    num_rolls = 1
    while num_rolls <= 10:
        new_score = make_averaged(roll_dice, total_samples)(num_rolls, dice)
        if new_score > biggest_total:
            biggest_total = new_score 
            biggest_roll = num_rolls
        num_rolls += 1
    return biggest_roll
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hefty_hog_score = hefty_hogs(score, opponent_score)
    hog_pile_score = hog_pile(hefty_hog_score + score, opponent_score)
    if hog_pile_score != 0 or hefty_hog_score >= threshold:
        return 0
    return num_rolls
    # END PROBLEM 11
